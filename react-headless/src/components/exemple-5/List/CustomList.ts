...
export const useCustomList = <ItemType>(items: ItemType[]) => {
  const listStoreRef = React.useRef<CustomList<ItemType>>();

  if (!listStoreRef.current) {
    listStoreRef.current = new CustomList(items);
  }

  return listStoreRef.current;
};
